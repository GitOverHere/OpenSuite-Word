    ...
    public function login($email, $password, $remember)
    {
		...
		$row= $q->fetch();
		     ...
		     $s->set('email',$email);
				 if($remember == "yes"){
				 $c= new Cookie();
				 $token = Hasher::random(16);
				 $c->set($this->cookie_name,$token,$expires);
				 $c->set('email',$token,$expires);
				 $this->_db->Add('sessions',array('user_id','email','hash'),array($row['id'],$email,Hasher::make($token)));
...
public function email_valid($email){
	...
	public static function login_token_valid($email) {
		$sql = "SELECT hash from sessions WHERE email = :email";
		$query = $this->_db->query($sql);
		$query->bindParam(":email",$hash);
		$query->execute();
		$row = $q->fetch();
		
		if(password_verify(Cookie::get("token"),$row["hash"])){
			return true;
		}
		else {
			return false;
